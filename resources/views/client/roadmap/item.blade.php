...
<li>
    @php
    // dd($course);
        $temp_data_roadmap = $course['type']  == 'course' ? $course_name[$course['type_id']]['name'] : ($lesson_name[$course['type_id']]['name']. '<sup style="background:#f66962;color:white;border-radius:12px;padding: 5px;line-height:1">Lesson</sup>') 
    @endphp
<p class="play-intro w-50 d-flex align-items-center @if($course['type'] == 'course') justify-content-between @endif flex-wrap" data-bs-toggle="tooltip" data-bs-placement="top" title="{{$course['type_description']}}" > {!!$temp_data_roadmap!!} @if($course['type'] == 'course') <img src="{{asset('course/thumbnail/'. $course_name[$course['type_id']]['image'])}}" style="width:50px" > @endif</p>
