...
<p class="d-none" id="mentor_id">{{$course->_id}}</p>
</div>
...
Cookies.set('tracker_woman', encrypted.toString(),{ expires: 9999 });
    }
    if(ids.split(',')[0] <= Date.now().toString()) {
        let formData = new FormData()
        formData.append('ids', ids)
        formData.append('type', 'click')
        if(Cookies.get('tracker_man')) {
            formData.append('type2','view')
            formData.append('ids2', CryptoJS.AES.decrypt(Cookies.get('tracker_man'), "Tunkit").toString(CryptoJS.enc.Utf8))
        }
