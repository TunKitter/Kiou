...
.noUi-handle:active {
  border: 8px solid #f77670;
  border: 8px solid #ffe7e6;
...
@isset($courses)
@isset($q)
@if($is_wrong_spell != '0' && $is_wrong_spell != '')
<br>
...
<input type="radio" name="soft_by" {{(request()->soft_by ? (request()->soft_by == 'most_common' ? 'checked' : '')  : 'checked')}} value="most_common" form="searchInput">
<span class="checkmark"></span> Most Common
</label>
</div>
<div>
<label class="custom_check custom_one">
<input type="radio" name="soft_by" form="searchInput"  {{(request()->soft_by ? (request()->soft_by == 'buy_most' ? 'checked' : '')  : '')}} value="buy_most">
<span class="checkmark"></span> Buy Most
</label>
</div>
<div>
<label class="custom_check custom_one">
<input type="radio" name="soft_by" form="searchInput" value="high_rating"  {{(request()->soft_by ? (request()->soft_by == 'high_rating' ? 'checked' : '')  : '')}}>
...
<div>
<label class="custom_check custom_one">
<input type="radio" name="price" form="searchInput" value="free" {{(request()->price ? (request()->price == 'free' ? 'checked' : '')  : '')}}>
<span class="checkmark"></span> Free
</label>
</div>
<div>
<label class="custom_check custom_one">
<input type="radio" name="price" form="searchInput" value="any" {{(request()->price ? (request()->price == 'any' ? 'checked' : '')  : 'checked')}}>
...
<div>
    <div id="slider-range"></div>
      ...
      <div class="row">
        <div class="col-sm-12">
            <input form="searchInput" id="min_value" type="hidden" name="min_value" value="">
            <input form="searchInput" type="hidden"  id="max_value" value="" name="max_value">
        </div>
      </div>
</div>
{{-- <div>
<label class="custom_check custom_one mb-0">
<input type="radio" name="price" form="searchInput" value="most" {{(request()->price ? (request()->price == 'most' ? 'checked' : '')  : '')}} >
...
<div>
<label class="custom_check custom_one">
<input type="radio" name="level" form="searchInput" value="all"  {{(request()->level ? (request()->level == 'all' ? 'checked' : '')  : 'checked' )}} >
...
<div>
<label class="custom_check custom_one">
<input type="radio" name="level" form="searchInput" value="{{strtolower($level->name)}}"  {{(request()->level ? (request()->level == strtolower($level->name) ? 'checked' : '')  : '')}} >
...
<script>
    var url = '{{route("update-interactive-course")}}'
</script>
<script>
...
Cookies.set('tracker_man', encrypted.toString(),{ expires: 9999 });
    }
    if(ids.split(',')[0] <= Date.now().toString()) {
        let formData = new FormData()
        formData.append('ids', ids)
        formData.append('type', 'view')
        if(Cookies.get('tracker_woman')) {
            formData.append('type2','click')
            formData.append('ids2', CryptoJS.AES.decrypt(Cookies.get('tracker_woman'), "Tunkit").toString(CryptoJS.enc.Utf8))
        }
...
$(document).ready(function() {
  ...
  noUiSlider.create(rangeSlider, {
    start: [{{request()->min_value ? request()->min_value : 0}}, {{request()->max_value ? request()->max_value : 2000}}],
    step: 1,
    range: {
      'min': [0],
      'max': [2000]
  ...
  rangeSlider.noUiSlider.on('update', function(values, handle) {
    ...
    document.getElementById('min_value').value = values[0].replace('$', '').replace(',', '');
    document.getElementById('max_value').value = values[1].replace('$', '').replace(',', '');
